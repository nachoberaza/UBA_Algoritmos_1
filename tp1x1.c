...
float const Z = (ANCHO) / (2 * 1); //la tangente de (90°/2) es = a 1

float d[3] = {-ANCHO / 2, -ALTO / 2, Z};


float producto_interno(float d0, float d1, float d2)
{
    CD = (CX * d0) + (CY * d1) + (CZ * d2);
...
int main(void)
{
    ...
    for (d[0]; d[0] >= 640; d[0]++)
    { //no se si asi se cambia el valor de un vector
        for (d[1]; d[1] <= 480; d[1]++)
        {
